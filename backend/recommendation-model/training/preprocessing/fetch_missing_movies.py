...
def validate_match(movie_id: str, title: str, year: str, api_title: str, api_ori_title: str, api_date: str) -> bool:
    ...
    api_year = api_date.split('-')[0] if api_date else None
    ...
    if year and api_year and year != api_year:
        print(f'Year mismatch for {movie_id}: {year} vs {api_year}')
        return False
...
async def fetch_omdb_json(session, imdb_id: str):
    fetch_url = f'http://www.omdbapi.com/?i=tt{imdb_id}&apikey={OMDB_API_KEY2}'
